...
const EXCEPTIONS: string[] = [
  ...
  /**
   * This translation key shows up in a JSDoc comment example in app/components/Text.tsx
   * It's documentation, not actual usage, so we exclude it from the test.
   */
  "common.welcome",
]
 ...
 * and modified slightly to account for our Shipnative higher order components,
...
describe("i18n", () => {
  test("There are no missing keys", (done) => {
    ...
    exec(command, (error, stdout) => {
      // Handle error or no output gracefully
      if (error && !stdout) {
        // If grep finds nothing, it exits with code 1, but that's okay
        done()
        return
      }
